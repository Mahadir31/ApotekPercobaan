...
Public Class Penjualan_Obat
    ...
    Sub NoOtomatis()
        Call OpenConn()
        Da = New MySqlDataAdapter("Select * from penjulan", Conn)
        Ds = New DataSet
        Da.Fill(Ds, "penjulan")
        Dgpenjualn.DataSource = Ds.Tables("penjulan")
        Dgpenjualn.AlternatingRowsDefaultCellStyle.BackColor = Color.Silver

        Call OpenConn()
        Cmd = New MySqlCommand("select no_penjualan from penjulan order by no_penjualan desc", Conn)
    ...
    Private Sub Penjualan_Obat_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        KodePetugas.Text = FormLogin.KdPetugas.Text
    End Sub
    ...
    Private Sub Btnsimpan_Click(sender As Object, e As EventArgs) Handles Btnsimpan.Click
        Try
            ...
            Dim InputPenjualan As String = "insert into penjulan values ('" & NoPenjualan.Text & "','" & tglmysql & "','" & KodeObat.Text & "', '" & CbNamaObat.Text & "','" & Format(DtPenjualan.Value, "yyyy-MM-dd") & "', '" & HargaObat.Text & "', '" & JenisObat.Text & "','" & JumlahObat.Text & "','" & KodePetugas.Text & "')"
            ...
            Call OpenConn()
            Dim InputDetailPenjualan As String = "insert into detailpenjualan values ('" & NoPenjualan.Text & "','" & JumlahObat.Text & "','" & TotalHarga.Text & "','" & KodePembeli.Text & "')"
